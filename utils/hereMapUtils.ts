...
export function createHereMap(
  ...
  const satelliteProvider = new H.map.provider.ImageTileProvider({
    ...
    getURL: (col: number, row: number, level: number) =>
      `https://maps.hereapi.com/v3/base/mc/${level}/${col}/${row}/png?style=logistics.satellite.day&features=vehicle_restrictions:active_and_inactive&ppi=72&apiKey=${HERE_MAP_API_KEY}`,
  });
