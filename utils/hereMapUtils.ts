...
const HERE_MAP_API_KEY = 'lkBYh3TgXFetMq_8bc0IFPVVDf68iAjzvdzjPNWuj9o';
...
export function getHerePlatform() {
  if (!_platform) {
    ...
    _defaultLayers = _platform.createDefaultLayers({ engineType: H.Map.EngineType.HARP });
  }
...
export function createHereMap(
  ...
  // Create the style object from the night theme configuration (Harp)
  ...
  // Create a layer with the style object using the OMV service
  const vectorLayer = platform.getOMVService().createLayer(style, { engineType: H.Map.EngineType.HARP });

  const map = new H.Map(element, vectorLayer, {
    center,
    zoom,
  ...
  // Native HERE logistics.satellite.day — no CSS filter applied
  ...
  // Add Leaflet-like compatibility shims to avoid "map.off is not a function" errors
  (map as any).on = function(type: string, handler: any) {
    const hereType = type === 'moveend' ? 'mapviewchangeend' : type;
    const wrappedHandler = (e: any) => {
      // Add latlng property for Leaflet compatibility
      if (e.viewportX !== undefined && !e.latlng) {
        e.latlng = this.screenToGeo(e.viewportX, e.viewportY);
      }
      handler(e);
    };
    handler._hereWrapped = wrappedHandler;
    this.addEventListener(hereType, wrappedHandler);
    return this;
  };

  (map as any).off = function(type: string, handler: any) {
    const hereType = type === 'moveend' ? 'mapviewchangeend' : type;
    if (handler && handler._hereWrapped) {
      this.removeEventListener(hereType, handler._hereWrapped);
    } else if (handler) {
      this.removeEventListener(hereType, handler);
    }
    return this;
  };

  (map as any).getBounds = function() {
    const rect = this.getViewBounds();
    if (!rect) return {
      getWest: () => 0, getSouth: () => 0, getEast: () => 0, getNorth: () => 0
    };
    return {
      getWest: () => rect.getLeft(),
      getSouth: () => rect.getBottom(),
      getEast: () => rect.getRight(),
      getNorth: () => rect.getTop(),
    };
  };
...
export function createDomMarker(
  ...
  zIndex?: number
): { marker: any; element: HTMLElement } {
  let el: HTMLElement;
  ...
  const marker = new H.map.DomMarker({ lat, lng }, { icon, zIndex: zIndex || 0 });
...
/** Show a HERE InfoBubble (equivalent of L.popup) */
export function showInfoBubble(map: any, lat: number, lng: number, content: string): any {
  const ui = (map as any).ui;
  if (!ui) return null;
