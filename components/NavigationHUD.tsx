...
const LaneRibbon: React.FC<{ parseLane: (step: any) => any; nextInstruction: any }> = ({ parseLane, nextInstruction }) => {
  const lanes = useMemo(() => {
    ...
    return rawLanes.map((lane: any) => {
      ...
      return {
        ...
        truckLane: lane.truckLane || parsed?.truckLane || false,
      };
    });
  ...
  const hasTruckLanes = lanes.some((l: any) => l.truckLane);

  return (
    <div data-testid="lane-ribbon" className="border-t border-[#D4AF37]/30 px-3 py-2 bg-black/60">
      <div className="flex flex-col gap-1 mb-1">
        {hasRecommendation && (
          <div className="text-[9px] uppercase tracking-wider text-[#D4AF37] text-center font-bold">
            Lane Guidance
          </div>
        )}
        {hasTruckLanes && (
          <div className="text-[8px] uppercase tracking-widest text-emerald-400 text-center font-black animate-pulse">
            Truck Lane Upcoming
          </div>
        )}
      </div>
      <div className="flex justify-center gap-0.5">
        {lanes.map((lane: any, idx: number) => (
          <div key={idx} 
            ...
            className={`flex flex-col items-center justify-center py-1 flex-1 max-w-[44px] transition-all rounded-md relative ${
              lane.active 
                ? 'bg-[#D4AF37]/15 ring-1 ring-[#D4AF37]/40' 
                : 'bg-white/[0.02]'
            }`}>
            {lane.truckLane && (
              <div className="absolute -top-1 left-1/2 -translate-x-1/2 bg-emerald-500 text-black text-[6px] font-black px-1 rounded-sm z-10">
                TRUCK
              </div>
            )}
