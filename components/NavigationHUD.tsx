...
import { Navigation, ArrowUp, ArrowRight, ArrowLeft, ArrowUpRight, ArrowUpLeft, RotateCcw, GitFork, Merge } from 'lucide-react';
  ...
  return (
    ...
    >
      <div className={`bg-[#1a1a2e]/92 backdrop-blur-2xl rounded-2xl shadow-[0_8px_32px_rgba(0,0,0,0.6)] border overflow-hidden transition-all duration-300 ${
        detailLevel === 'immediate' ? 'border-[#E259AD]/50 shadow-[0_0_24px_rgba(226,89,173,0.25)]' : 'border-white/8'
      }`}>
        {/* Main maneuver row */}
        <div className="flex items-stretch">
          {/* Big direction arrow panel */}
          <div className={`flex flex-col items-center justify-center px-5 py-4 min-w-[88px] ${
            detailLevel === 'immediate' ? 'bg-[#E259AD]/15' : 'bg-white/[0.04]'
          }`}>
            ...
            {dist && (
              <div className="flex items-baseline gap-0.5 mt-1">
                <span className={`text-2xl font-[900] tracking-tight leading-none tabular-nums ${
                  detailLevel === 'immediate' ? 'text-[#E259AD]' : 'text-white'
                }`}>
                  {dist.value}
                </span>
...
const LaneArrow: React.FC<{ direction: string; active: boolean }> = ({ direction, active }) => {
  ...
  const color = active ? '#E259AD' : 'rgba(255,255,255,0.12)';
        ...
        {lanes.map((lane: any, idx: number) => (
          <div key={idx} 
            ...
            className={`flex flex-col items-center justify-center py-1 flex-1 max-w-[44px] transition-all rounded-md ${
              lane.active 
                ? 'bg-[#E259AD]/15 ring-1 ring-[#E259AD]/30' 
                : 'bg-white/[0.02]'
            }`}>
            <LaneArrow direction={lane.direction} active={lane.active} />
            {lane.active && (
              <div className="w-1.5 h-1.5 bg-[#E259AD] rounded-full mt-0.5 shadow-[0_0_4px_rgba(226,89,173,0.6)]" />
            )}
          </div>
        ))}
      </div>
