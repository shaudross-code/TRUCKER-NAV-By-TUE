...
import { calcDist, calcEuclideanDist, convertInstructionToImperial, synthesizeLanes } from '../utils/navigationHelpers';
...
// Helpers extracted to /app/utils/navigationHelpers.ts
...
const NavigationView: React.FC<NavigationViewProps> = ({ initialTarget, userLocation: propUserLocation, activeView }) => {
  ...
  // Night Mode Auto-Dimming (P2)
  const [nightModeEnabled, setNightModeEnabled] = useState(() => {
    const saved = uGet('nav_night_mode');
    return saved === null ? true : saved === 'true'; // Enabled by default
  });
  const { isNightMode, brightness, hudOpacity, mapFilter: nightMapFilter } = useNightMode({
    enabled: nightModeEnabled,
    latitude: userLocation?.[0],
  });

  // Viewport-based sign culling (P2)
  const { isInViewport, filterVisible, updateBounds: updateViewportBounds } = useViewportCulling(mapInstanceRef);
  ...
  useEffect(() => {
    ...
    // Step 7: Viewport culling — only render markers in the visible viewport
    const visible = capped.filter(item => isInViewport(item.position[0], item.position[1], 0.02));

    visible.forEach((item: TrafficInfrastructure) => {
      try {
        const iconHtml = renderToStaticMarkup(
          <TrafficIcon
            type={item.type}
          ...
          <div className="w-full h-full overflow-hidden relative">
            <div id="nav-map-container" ref={mapRef} className={`absolute transition-opacity duration-500`}
              style={{ 
                ...
                filter: isNightMode ? nightMapFilter : undefined,
